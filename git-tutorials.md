...
``` bash
git restore --staged "파일 또는 폴더"
```

\#\# 참고 2 : 현재 git 작업상태 확인 명렁어
...
협업할 때 원본 파일을 직접 수정하면 

다른 사람의 작업과 꼬이는 문제가 생길 수 있습니다.

그래서 **원본은 그대로 두고, 나만의 가상 작업 공간(Branch)**을 따로 만들어 작업한 후

완성된 수정본만 합치는 방식을 사용합니다.

<br>

여기까지 무슨말인지 잘 감이 안 오실 수도 있습니다.

아래 예시들을 하나하나 해보시면 조금 이해가 되실수도 있을 거에요.
...
사실 레포지토리 생성 명령어를 통해 레포지토리를 새로 만드는 과정에서

메인브랜치가 `main`이라는 기본값 이름으로 설정이 됩니다.

시스템에 따라 `master`가 기본값인 경우도 있습니다.

아래 명령어를 통해 기본값이 무엇인지 확인할 수 있습니다.

``` bash
git config --get init.defaultBranch
```

물론 뒤에 나오는 명령어로 이름을 바꿀 수도 있습니다.
...
![Branch.png](githubimgs/branch2.png)

위 그림의 빨간 사각형이 메인브랜치 입니다.

메인브랜치는 **프로젝트를 처음 만들면 자동으로 생기는 기본 줄기** 정도로 생각하면 될 것 같습니다.

레포지토리를 만들 때 처음 생기는 브랜치이기도 하지만

많은 사람들이 협업할 때 최종 작업물로 활용하는 공간이기도 합니다.

메인브랜치 안의 내용들은 최종 결과물이 되야 하기 때문에

협업을 할 때는 메인브랜치 안을 직접 수정하기보다는

개인 작업공간 브랜치를 따로 만들어 수정본을 만들고

나중에 검토 후 메인브랜치에 반영하는 방식입니다.

-----------------------------------------------------------------------------------------

### 브랜치 이해하기

레포지토리 A 아래에 다음과 같이 폴더 및 파일이 있다고 해봅시다.

![branch3.png](githubimgs/branch3.png)

만약 사용자가 D 폴더로 이동해서 `new`라는 브랜치를 만들었다고 해봅시다.

여기서 오해하면 안 되는 것이 있습니다.

D 폴더에서 `new`를 만들었다고

`new` 안의 구조가 D폴더, H폴더로만 구성되어있다고 오해하시면 안됩니다.

`new` 안의 구조는 위 그림과 동일합니다.

만약 이 안에서 D 폴더 안에 I 폴더를 새로 만들고, 그 안에 `i.py`라는 파일을 새로 만들었다면

`new` 안의 구조는 아래와 같이 됩니다.

![branch4.png](githubimgs/branch4.png)

만약 여기서 다시 `main` 브랜치로 이동한다면 구조는 아래와 같이 다시 변경됩니다.

![branch3.png](githubimgs/branch3.png)

즉 `new`안에서 새로 만들었던 I 폴더와 `i.py`은 

기존에 있었던 `main`브랜치에서는 보여주지도, 반영되지도 않는다는 말입니다.
...
주의 : 위 명령어는 내 컴퓨터에만 브랜치가 생성됩니다.

git 저장소 상에서는 아직 브랜치가 생성되지 않은 상태입니다.

나중에 업로드(푸시) 할 때 아래 명령어 방식으로 똑같은 이름의 브랜치를 git상에 만들어 연결해야 합니다. (첫 1회만 하면 됩니다.)

``` bash
git push -u "원격저장소 별명" "새로만든 브랜치 이름"
```
...
|옵션|의미|설명|
|---|:---:|:---:|
|`-d`|안전한 삭제|메인에 합쳐지기 전에 삭제 시도하는 경우, 경고를 띄움
|`-D`|강제 삭제|경고 없이 무조건 삭제|

주의 : 마찬가지로 위 명령어 2개는 모두 내 컴퓨터 상에서만 브랜치를 삭제합니다.

git에 있는 브랜치를 삭제하진 않습니다.

git에 있는 브랜치를 삭제하려면 아래와 같은 명령어를 사용합니다.

``` bash
git push "원격저장소 이름" --delete "브랜치 이름"
```
...
자신이 현재 어떤 브랜치에 있는지 확인할 때 사용합니다.

``` bash
git branch
```

혹시 업로드 3종 명령어 기억하시나요?

좀 더 정확히 말하면, 업로드 3종 명령어를 실행하면

파일 또는 디렉토리를 **'현재 브랜치에 추가'**하는 것입니다.
