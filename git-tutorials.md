...
`clone`은 git에서 내 로컬 저장소로 파일을 다운받는 명령어입니다.
...
----------------------------------------------------------------------
## 풀(pull)
위에서 우리는 git에 업로드하는 명령어들을 알아봤습니다.

사실 업로드할 때 변경 내용들도 같이 올라갑니다.

히스토리 라고도 불리는데요.

변경 기록들 정도로 생각하시면 됩니다.

`pull`이라는 명령어를 통해 파일이나 디렉토리를 다운받을 수도 있습니다.

`clone`과의 차이점은 `pull`은 이 히스토리까지 받아옵니다.
...
문제는 내 로컬의 히스토리와 깃의 히스토리가 일치하지 않는 경우에는 문제가 발생합니다.
